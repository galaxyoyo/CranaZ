...
public class Loots extends Loadable{
					...
					if(sender instanceof Player)
					{
						if(sender.hasPermission("clp.pack.delete")){
							...
							if(factor.deleteSpawnPoint(factor.PACK, player.getTargetBlock(null, 50).getLocation()))
								gsay(player, "Point de pack supprimé.");
							else
				...
				else if(args[0].equalsIgnoreCase("info"))
				{
					if(args.length > 1)
					{
						...
						case "hopital":
							...
							break;
						default:
							sendMessages((Player)sender, "Le pack spécifié n'existe pas !");
						}
					}
					else
						showHelp(sender);
	...
	{
		warn(player, "Veuillez spécifier un pack valide !");
	}
	...
	public void showHelp(CommandSender sender)
	{
		...
		sender.sendMessage("/clp get [objet] - Recuperer un objet personnalisé. Ne rien mettre pour la liste des objets.");
